# RapidKit Connector Hub Module

External app connector scaffolding for OAuth credentials, provider adapters, sync jobs, and health
checks.
...
- Defines provider adapters behind a stable connector interface.
- Stores credential profiles separately from sync state.
- Schedules sync jobs through the workflow/task layer.
- Surfaces connector health and audit hooks.

## Install Commands

```bash
rapidkit add module connector_hub
...
```bash
rapidkit add module free/business/connector_hub
```

## Quickstart

1. Register provider adapters and supported scopes.
1. Store OAuth credentials through the auth module.
1. Schedule sync jobs through workflow execution.
1. Report connector health to admin/support surfaces.
...
| ------------------------------------- | ------------------------------------------------- |
| `RAPIDKIT_CONNECTOR_HUB_ENABLED`      | Enables generated connector services.             |
| `RAPIDKIT_CONNECTOR_HUB_SYNC_MODE`    | Controls manual, scheduled, or event-driven sync. |
| `RAPIDKIT_CONNECTOR_HUB_SECRET_SCOPE` | Secret namespace for provider credentials.        |
...
- Audit scope changes and manual sync actions.
- Handle provider rate limits and revocation explicitly.
...
```bash
poetry run pytest tests/modules/free/business/connector_hub -q
poetry run python -m modules.free.business.connector_hub.generate fastapi ./tmp/connector_hub-fastapi
poetry run python -m modules.free.business.connector_hub.generate nestjs ./tmp/connector_hub-nestjs
poetry run python scripts/validate_module_structure.py free/business/connector_hub
poetry run python scripts/validate_module_docs_quality.py --module free/business/connector_hub
poetry run python scripts/validate_module_snippet_configs.py --module free/business/connector_hub
```
...
Category: `business` Canonical slug: `free/business/connector_hub`
