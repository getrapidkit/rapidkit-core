# RapidKit Forms Engine Module

Schema-driven forms engine with validation, submissions, workflow hooks, and audit-ready exports.
...
- Defines versioned form schemas and validation rules.
- Captures submissions with tenant, actor, and workflow context.
- Supports admin-controlled schema rollout and deprecation.
- Exports audit-ready submission records for operations teams.

## Install Commands

```bash
rapidkit add module forms_engine
...
```bash
rapidkit add module free/business/forms_engine
```

## Quickstart

1. Define form schemas as versioned configuration.
1. Validate submissions before workflow or billing side effects.
1. Record schema version and actor context with each submission.
1. Route sensitive submissions through audit and retention policy.
...
| ------------------------------------ | --------------------------------------------------- |
| `RAPIDKIT_FORMS_ENGINE_ENABLED`      | Enables generated form services.                    |
| `RAPIDKIT_FORMS_ENGINE_SCHEMA_MODE`  | Controls strict or compatibility schema validation. |
| `RAPIDKIT_FORMS_ENGINE_EXPORT_LIMIT` | Maximum records returned by export operations.      |
...
- Do not trust client-side validation as authorization.
- Redact sensitive fields in logs and exports.
- Require elevated permission for schema changes and bulk exports.
...
```bash
poetry run pytest tests/modules/free/business/forms_engine -q
poetry run python -m modules.free.business.forms_engine.generate fastapi ./tmp/forms_engine-fastapi
poetry run python -m modules.free.business.forms_engine.generate nestjs ./tmp/forms_engine-nestjs
poetry run python scripts/validate_module_structure.py free/business/forms_engine
poetry run python scripts/validate_module_docs_quality.py --module free/business/forms_engine
poetry run python scripts/validate_module_snippet_configs.py --module free/business/forms_engine
```
...
Category: `business` Canonical slug: `free/business/forms_engine`
