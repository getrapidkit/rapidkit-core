# RapidKit Feature Flags Module

Rollout, entitlement gates, experiments, and kill switches for productized workspaces.
...
- Evaluates flags by tenant, user, role, and entitlement.
- Supports staged rollout and emergency kill switches.
- Keeps product experiments isolated from deployment.
- Provides audit hooks for flag changes.

## Install Commands

```bash
rapidkit add module feature_flags
...
```bash
rapidkit add module free/business/feature_flags
```

## Quickstart

1. Define flag keys and ownership.
1. Evaluate flags at service boundaries.
1. Connect paid features to entitlement checks.
1. Audit production flag changes.
...
| ------------------------------------- | --------------------------------------- |
| `RAPIDKIT_FEATURE_FLAGS_ENABLED`      | Enables generated flag evaluation.      |
| `RAPIDKIT_FEATURE_FLAGS_DEFAULT_MODE` | Default allow, deny, or audit behavior. |
| `RAPIDKIT_FEATURE_FLAGS_CACHE_TTL`    | Cache TTL for flag definitions.         |
...
- Default closed for paid or sensitive capabilities.
- Do not use client-side flags as authorization.
- Record operator, reason, and previous value for changes.
...
```bash
poetry run pytest tests/modules/free/business/feature_flags -q
poetry run python -m modules.free.business.feature_flags.generate fastapi ./tmp/feature_flags-fastapi
poetry run python -m modules.free.business.feature_flags.generate nestjs ./tmp/feature_flags-nestjs
poetry run python scripts/validate_module_structure.py free/business/feature_flags
poetry run python scripts/validate_module_docs_quality.py --module free/business/feature_flags
poetry run python scripts/validate_module_snippet_configs.py --module free/business/feature_flags
```
...
Category: `business` Canonical slug: `free/business/feature_flags`
