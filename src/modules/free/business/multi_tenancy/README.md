# RapidKit Multi Tenancy Module

Organization, team, membership, tenant context, and isolation scaffolding for SaaS products.
...
- Introduces tenant-aware request context.
- Models organizations, teams, and memberships.
- Provides hooks for tenant-scoped persistence and authorization.
- Supports admin boundaries for product operations.

## Install Commands

```bash
rapidkit add module multi_tenancy
...
```bash
rapidkit add module free/business/multi_tenancy
```

## Quickstart

1. Create organizations and memberships during onboarding.
1. Resolve tenant context before domain handlers run.
1. Scope database reads and writes by tenant.
1. Propagate tenant IDs into logs, metrics, and audit events.
...
| -------------------------------- | ----------------------------------------------- |
| `RAPIDKIT_MULTI_TENANCY_ENABLED` | Enables tenant-aware services and guards.       |
| `RAPIDKIT_MULTI_TENANCY_HEADER`  | Header used to resolve explicit tenant context. |
| `RAPIDKIT_MULTI_TENANCY_STRICT`  | Rejects requests without a resolvable tenant.   |
...
- Never trust a tenant header without user membership validation.
- Test cross-tenant access attempts in every product.
- Include tenant IDs in audit records for privileged actions.
...
```bash
poetry run pytest tests/modules/free/business/multi_tenancy -q
poetry run python -m modules.free.business.multi_tenancy.generate fastapi ./tmp/multi_tenancy-fastapi
poetry run python -m modules.free.business.multi_tenancy.generate nestjs ./tmp/multi_tenancy-nestjs
poetry run python scripts/validate_module_structure.py free/business/multi_tenancy
poetry run python scripts/validate_module_docs_quality.py --module free/business/multi_tenancy
poetry run python scripts/validate_module_snippet_configs.py --module free/business/multi_tenancy
```
...
Category: `business` Canonical slug: `free/business/multi_tenancy`
