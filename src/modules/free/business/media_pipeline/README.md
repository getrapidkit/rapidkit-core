# RapidKit Media Pipeline Module

Media ingestion and transformation pipeline with checksum, variant generation, and moderation hooks.
...
- Models media ingestion, validation, transformation, and variant generation.
- Tracks checksums, source metadata, and generated asset lineage.
- Provides hooks for moderation and policy enforcement.
- Reports processing state for support and admin dashboards.

## Install Commands

```bash
rapidkit add module media_pipeline
...
```bash
rapidkit add module free/business/media_pipeline
```

## Quickstart

1. Upload media through storage with checksum and content-type validation.
1. Generate variants through background workers.
1. Attach tenant and source metadata to every derived asset.
1. Moderate or quarantine assets before external publication.
...
| ----------------------------------------- | ---------------------------------------------- |
| `RAPIDKIT_MEDIA_PIPELINE_ENABLED`         | Enables generated media processing services.   |
| `RAPIDKIT_MEDIA_PIPELINE_MAX_BYTES`       | Maximum accepted media object size.            |
| `RAPIDKIT_MEDIA_PIPELINE_MODERATION_MODE` | Controls audit, quarantine, or block behavior. |
...
- Treat media metadata and payloads as untrusted input.
- Do not publish generated variants before policy checks pass.
- Limit transformation concurrency to protect worker capacity.
...
```bash
poetry run pytest tests/modules/free/business/media_pipeline -q
poetry run python -m modules.free.business.media_pipeline.generate fastapi ./tmp/media_pipeline-fastapi
poetry run python -m modules.free.business.media_pipeline.generate nestjs ./tmp/media_pipeline-nestjs
poetry run python scripts/validate_module_structure.py free/business/media_pipeline
poetry run python scripts/validate_module_docs_quality.py --module free/business/media_pipeline
poetry run python scripts/validate_module_snippet_configs.py --module free/business/media_pipeline
```
...
Category: `business` Canonical slug: `free/business/media_pipeline`
