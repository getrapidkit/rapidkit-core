# RapidKit Cors Module

Cross-Origin Resource Sharing security module
...
- Provide a configurable CORS policy with consistent defaults across FastAPI and NestJS.
- Support allow-origins/methods/headers/credentials with generation-time overrides.
- Ship health metadata so CORS policy state can be surfaced in unified `src/health` output.
...
Document the environment variables or override contracts exposed by `overrides.py`. Include a table
similar to:

| Environment Variable              | Effect                                                    |
| --------------------------------- | --------------------------------------------------------- |
| `RAPIDKIT_CORS_ENABLED`           | Enable/disable CORS wiring (bool).                        |
| `RAPIDKIT_CORS_ALLOW_ORIGINS`     | Comma-separated list or JSON array of allowed origins.    |
| `RAPIDKIT_CORS_ALLOW_METHODS`     | Allowed methods (comma-separated or JSON array).          |
| `RAPIDKIT_CORS_ALLOW_HEADERS`     | Allowed request headers (comma-separated or JSON array).  |
| `RAPIDKIT_CORS_EXPOSE_HEADERS`    | Exposed response headers (comma-separated or JSON array). |
| `RAPIDKIT_CORS_ALLOW_CREDENTIALS` | Allow credentialed requests (bool).                       |
| `RAPIDKIT_CORS_MAX_AGE`           | Preflight cache max-age seconds (int).                    |
| `RAPIDKIT_CORS_LOG_LEVEL`         | Override log level for CORS diagnostics.                  |
| `RAPIDKIT_CORS_METADATA`          | JSON metadata merged into health payload.                 |
...
```bash
poetry run pytest tests/modules/free/security/cors -q

poetry run python scripts/check_module_integrity.py --module free/security/cors
```
