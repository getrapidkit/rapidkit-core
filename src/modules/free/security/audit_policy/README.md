# RapidKit Audit Policy Module

Immutable audit events, reason capture, admin policy, and compliance export scaffolding.
...
- Captures privileged actions with actor, target, reason, and context.
- Provides tamper-evident event shapes for compliance workflows.
- Adds reusable policy hooks for admin actions.
- Supports export-oriented audit records.

## Install Commands

```bash
rapidkit add module audit_policy
...
```bash
rapidkit add module free/security/audit_policy
```

## Quickstart

1. Wrap admin commands with audit policy checks.
1. Capture reason and request context before mutation.
1. Write immutable event records after successful action.
1. Export audit history for support and compliance review.
...
| -------------------------------------- | -------------------------------------------- |
| `RAPIDKIT_AUDIT_POLICY_ENABLED`        | Enables audit policy hooks.                  |
| `RAPIDKIT_AUDIT_POLICY_REQUIRE_REASON` | Requires reason text for privileged actions. |
| `RAPIDKIT_AUDIT_POLICY_RETENTION_DAYS` | Retention target for audit records.          |
...
- Do not allow users to mutate audit records in place.
- Protect audit export endpoints with elevated permissions.
- Include correlation IDs for incident reconstruction.
...
```bash
poetry run pytest tests/modules/free/security/audit_policy -q
poetry run python -m modules.free.security.audit_policy.generate fastapi ./tmp/audit_policy-fastapi
poetry run python -m modules.free.security.audit_policy.generate nestjs ./tmp/audit_policy-nestjs
poetry run python scripts/validate_module_structure.py free/security/audit_policy
poetry run python scripts/validate_module_docs_quality.py --module free/security/audit_policy
poetry run python scripts/validate_module_snippet_configs.py --module free/security/audit_policy
```
...
Category: `security` Canonical slug: `free/security/audit_policy`
