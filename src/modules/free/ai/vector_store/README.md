# RapidKit Vector Store Module

Vector index abstraction for embeddings, metadata filters, and similarity search.
...
- Provides a stable boundary for pgvector/Qdrant-style backends.
- Stores embedding metadata needed for filtering and citation.
- Exposes health checks for index availability.
- Keeps vector search replaceable across products.

## Install Commands

```bash
rapidkit add module vector_store
...
```bash
rapidkit add module free/ai/vector_store
```

## Quickstart

1. Select a backend and configure connection details.
1. Create an index per tenant or apply tenant metadata filters.
1. Write embeddings through the module facade.
1. Query through similarity search with explicit limits.
...
| ---------------------------------- | ----------------------------------------------- |
| `RAPIDKIT_VECTOR_STORE_ENABLED`    | Enables generated vector store adapters.        |
| `RAPIDKIT_VECTOR_STORE_BACKEND`    | Selected backend adapter.                       |
| `RAPIDKIT_VECTOR_STORE_DIMENSIONS` | Embedding dimensionality expected by the index. |
...
- Treat metadata filters as part of authorization.
- Do not expose raw nearest-neighbor endpoints without rate limits.
...
```bash
poetry run pytest tests/modules/free/ai/vector_store -q
poetry run python -m modules.free.ai.vector_store.generate fastapi ./tmp/vector_store-fastapi
poetry run python -m modules.free.ai.vector_store.generate nestjs ./tmp/vector_store-nestjs
poetry run python scripts/validate_module_structure.py free/ai/vector_store
poetry run python scripts/validate_module_docs_quality.py --module free/ai/vector_store
poetry run python scripts/validate_module_snippet_configs.py --module free/ai/vector_store
```
...
Category: `ai` Canonical slug: `free/ai/vector_store`
