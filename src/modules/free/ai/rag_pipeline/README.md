# RapidKit RAG Pipeline Module

Document ingestion, embeddings, retrieval, citations, and evaluation-ready AI search scaffolding.
...
- Defines ingestion and chunking boundaries for knowledge products.
- Connects embeddings to a vector store abstraction.
- Surfaces retrieval metadata and citation-ready payloads.
- Includes health hooks for indexing and retrieval dependencies.

## Install Commands

```bash
rapidkit add module rag_pipeline
...
```bash
rapidkit add module free/ai/rag_pipeline
```

## Quickstart

1. Store source documents through the storage module.
1. Index documents with a background task.
1. Query the vector store through the generated retrieval facade.
1. Return citations with every generated answer.
...
| ---------------------------------- | --------------------------------------------------- |
| `RAPIDKIT_RAG_PIPELINE_ENABLED`    | Enables generated ingestion and retrieval surfaces. |
| `RAPIDKIT_RAG_PIPELINE_CHUNK_SIZE` | Default document chunk size.                        |
| `RAPIDKIT_RAG_PIPELINE_TOP_K`      | Default retrieval result count.                     |
...
- Apply tenant filters during indexing and retrieval.
- Keep source permissions attached to chunks.
- Avoid mixing private and public corpora in a single unrestricted index.
...
```bash
poetry run pytest tests/modules/free/ai/rag_pipeline -q
poetry run python -m modules.free.ai.rag_pipeline.generate fastapi ./tmp/rag_pipeline-fastapi
poetry run python -m modules.free.ai.rag_pipeline.generate nestjs ./tmp/rag_pipeline-nestjs
poetry run python scripts/validate_module_structure.py free/ai/rag_pipeline
poetry run python scripts/validate_module_docs_quality.py --module free/ai/rag_pipeline
poetry run python scripts/validate_module_snippet_configs.py --module free/ai/rag_pipeline
```
...
Category: `ai` Canonical slug: `free/ai/rag_pipeline`
