# RapidKit Agent Runtime Module

Agent execution scaffolding for tools, memory hooks, retries, and durable workflow state.
...
- Models agent runs as observable units of work.
- Separates tool registration from execution policy.
- Supports retry, cancellation, and state persistence hooks.
- Pairs with the workflow engine for long-running agent tasks.

## Install Commands

```bash
rapidkit add module agent_runtime
...
```bash
rapidkit add module free/ai/agent_runtime
```

## Quickstart

1. Define tool contracts before exposing them to agents.
1. Route all model calls through `llm_gateway`.
1. Persist run state for any task that can outlive a request.
1. Emit run lifecycle events for support and audit workflows.
...
| ------------------------------------ | ----------------------------------------------- |
| `RAPIDKIT_AGENT_RUNTIME_ENABLED`     | Enables generated agent endpoints and services. |
| `RAPIDKIT_AGENT_RUNTIME_MAX_STEPS`   | Maximum tool/model steps per run.               |
| `RAPIDKIT_AGENT_RUNTIME_RETRY_LIMIT` | Retry ceiling for transient execution failures. |
...
- Enforce tool allowlists per tenant or role.
- Record run metadata without storing sensitive prompt payloads by default.
...
```bash
poetry run pytest tests/modules/free/ai/agent_runtime -q
poetry run python -m modules.free.ai.agent_runtime.generate fastapi ./tmp/agent_runtime-fastapi
poetry run python -m modules.free.ai.agent_runtime.generate nestjs ./tmp/agent_runtime-nestjs
poetry run python scripts/validate_module_structure.py free/ai/agent_runtime
poetry run python scripts/validate_module_docs_quality.py --module free/ai/agent_runtime
poetry run python scripts/validate_module_snippet_configs.py --module free/ai/agent_runtime
```
...
Category: `ai` Canonical slug: `free/ai/agent_runtime`
