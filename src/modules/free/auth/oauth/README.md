...
Extend `src/auth/oauth.py` with:

| Customisation             | Purpose                                                              |
| ------------------------- | -------------------------------------------------------------------- |
| `get_provider_registry()` | Fetch provider configuration from secrets manager or config service. |
| `build_authorize_url()`   | Attach PKCE parameters or additional scopes per provider.            |
| `token_request_payload()` | Modify grant type or include provider-specific claims.               |
| Telemetry hooks           | Emit structured logs when sign-ins succeed or fail.                  |
...
```bash
poetry run pytest tests/modules/free/auth/oauth -q

poetry run python scripts/check_module_integrity.py --module free/auth/oauth
```
