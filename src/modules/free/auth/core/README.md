...
| -------------------------------- | ------------------------------------------------------------------------ |
| `RAPIDKIT_AUTH_CORE_HASH`        | Override PBKDF2 digest (default `sha256`).                               |
| `RAPIDKIT_AUTH_CORE_ITERATIONS`  | Override PBKDF2 iteration count.                                         |
| `RAPIDKIT_AUTH_CORE_SALT_BYTES`  | Control salt size (bytes) for password hashing.                          |
| `RAPIDKIT_AUTH_CORE_TOKEN_BYTES` | Control token entropy for opaque secrets.                                |
| `RAPIDKIT_AUTH_CORE_TOKEN_TTL`   | Change default token TTL (seconds).                                      |
| `RAPIDKIT_AUTH_CORE_ISSUER`      | Override issuer claim used in generated tokens.                          |
| `RAPIDKIT_AUTH_CORE_PEPPER_ENV`  | Switch the environment variable name used to resolve the signing pepper. |
| `RAPIDKIT_AUTH_CORE_POLICY`      | JSON object overriding password policy attributes.                       |

All overrides are composed by `AuthCoreOverrides`, ensuring constraints (minimum salt/TTL) remain
sane.
...
```bash
poetry run pytest tests/modules/free/auth/core -q

poetry run python scripts/check_module_integrity.py --module free/auth/core
```
