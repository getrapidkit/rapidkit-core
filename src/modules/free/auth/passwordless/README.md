...
| ---------------------------------- | ------------------------------------------------ |
| `PasswordlessRuntime` parameters   | Control token length, TTL, and retry policy.     |
| `send_code` callback               | Integrate with email/SMS providers.              |
| `store_token` / `load_token` hooks | Persist verification payloads in your datastore. |
| Telemetry callbacks                | Emit structured events for observability.        |
...
```bash
poetry run pytest tests/modules/free/auth/passwordless -q

poetry run python scripts/check_module_integrity.py --module free/auth/passwordless
```
