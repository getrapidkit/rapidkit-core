# RapidKit Cart Module

Shopping cart service for checkout flows
...
- Maintain rich cart snapshots with deterministic totals, discount applications, and tax
  calculation.
- Provide a validated service API (`CartService`) with in-memory persistence and extension hooks for
  external stores.
- Auto-generate framework bindings (FastAPI, NestJS) including REST handlers, dependency wiring, and
  JSON-friendly payloads.
- Expose a health surface that reports cart inventory, discount config status, and recent activity
  metrics.
- Offer configuration-driven behaviour via `config/base.yaml` and composable snippets for
...
| -------------------------------- | ----------------------------------------------------------------------------------------------- |
| `RAPIDKIT_CART_DEFAULT_DISCOUNT` | Overrides the default discount code applied to new carts when auto-application is enabled.      |
| `RAPIDKIT_CART_MAX_UNIQUE_ITEMS` | Caps the number of unique SKUs allowed per cart (defaults to `config/base.yaml`).               |
| `RAPIDKIT_CART_DISABLE_TAX`      | Forces tax totals to zero without modifying configuration files, useful for tax-exempt regions. |

Look inside `overrides.py` for additional hooks (including service factory overrides and health
payload enrichers).
...
```bash
poetry run pytest tests/modules/free/billing/cart -q

poetry run python scripts/check_module_integrity.py --module free/billing/cart
```
