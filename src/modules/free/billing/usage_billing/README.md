# RapidKit Usage Billing Module

Usage metering, quota enforcement, plan limits, and overage hooks for SaaS products.
...
- Records metered product usage in a provider-neutral shape.
- Evaluates quotas and plan limits before expensive operations.
- Connects usage events to entitlements and billing providers.
- Supports overage, warning, and hard-block policies.

## Install Commands

```bash
rapidkit add module usage_billing
...
```bash
rapidkit add module free/billing/usage_billing
```

## Quickstart

1. Define meters for product-specific billable events.
1. Check quota before model calls, exports, or automation runs.
1. Write usage events after successful work.
1. Reconcile usage with Stripe or the chosen billing provider.
...
| --------------------------------------- | ------------------------------------------ |
| `RAPIDKIT_USAGE_BILLING_ENABLED`        | Enables usage metering hooks.              |
| `RAPIDKIT_USAGE_BILLING_MODE`           | Controls audit, warn, or enforce behavior. |
| `RAPIDKIT_USAGE_BILLING_FLUSH_INTERVAL` | Batch interval for usage event export.     |
...
- Make usage writes idempotent.
- Keep billing decisions auditable.
- Treat quota bypasses as privileged admin actions.
...
```bash
poetry run pytest tests/modules/free/billing/usage_billing -q
poetry run python -m modules.free.billing.usage_billing.generate fastapi ./tmp/usage_billing-fastapi
poetry run python -m modules.free.billing.usage_billing.generate nestjs ./tmp/usage_billing-nestjs
poetry run python scripts/validate_module_structure.py free/billing/usage_billing
poetry run python scripts/validate_module_docs_quality.py --module free/billing/usage_billing
poetry run python scripts/validate_module_snippet_configs.py --module free/billing/usage_billing
```
...
Category: `billing` Canonical slug: `free/billing/usage_billing`
