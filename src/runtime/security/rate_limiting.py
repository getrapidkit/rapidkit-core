...
"""Runtime primitives for Rate Limiting.
...
_DEFAULT_NAMESPACE: Dict[str, Any] = json.loads(json.dumps(_DEFAULTS_RAW))
_DEFAULT_RULES: list[Dict[str, Any]] = json.loads(json.dumps(_DEFAULTS_RAW.get("rules", [])))
_HEADER_RAW: Mapping[str, str] = cast(Mapping[str, str], _DEFAULTS_RAW.get("headers", {}))
_HEADER_DEFAULTS = {
    "limit": _HEADER_RAW.get("limit", "X-RateLimit-Limit"),
    "remaining": _HEADER_RAW.get("remaining", "X-RateLimit-Remaining"),
    "reset": _HEADER_RAW.get("reset", "X-RateLimit-Reset"),
    "retry_after": _HEADER_RAW.get("retry_after", "Retry-After"),
    "rule": _HEADER_RAW.get("rule", "X-RateLimit-Rule"),
}

RateLimitScope = Literal["global", "identity", "route", "route-identity"]


def _coerce_scope(value: Any, default: RateLimitScope = "identity") -> RateLimitScope:
    cleaned = _as_str(value, default) or default
    allowed: tuple[RateLimitScope, ...] = ("global", "identity", "route", "route-identity")
    if cleaned in allowed:
        return cast(RateLimitScope, cleaned)
    return default
...
class RateLimitRule:
    ...
    @classmethod
    def from_mapping(
        cls, payload: Mapping[str, Any], *, default_name: str = "rule"
    ) -> "RateLimitRule":
        ...
        scope = _coerce_scope(payload.get("scope"), "identity")
...
class RateLimiterConfig:
    """Resolved configuration for Rate Limiting."""
    ...
    redis_prefix: str = (
        _as_str(_DEFAULT_NAMESPACE.get("redis_prefix"), "rate-limit") or "rate-limit"
    )
    trust_forwarded_for: bool = _as_bool(_DEFAULT_NAMESPACE.get("trust_forwarded_for"), False)
    forwarded_for_header: str = (
        _as_str(_DEFAULT_NAMESPACE.get("forwarded_for_header"), "X-Forwarded-For")
        or "X-Forwarded-For"
    )
    identity_header: str | None = _as_str(
        _DEFAULT_NAMESPACE.get("identity_header"), "X-RateLimit-Identity"
    )
    default_scope: RateLimitScope = _coerce_scope(
        _DEFAULT_NAMESPACE.get("default_scope"), "identity"
    )
    default_rule_name: str = (
        _as_str(_DEFAULT_NAMESPACE.get("default_rule_name"), "default") or "default"
    )
...
class RedisRateLimitBackend:
    ...
    def __init__(self, redis_url: str, *, prefix: str = "rate-limit") -> None:
        ...
        self._redis: AsyncRedisType = redis_async.Redis.from_url(redis_url, decode_responses=False)
...
def _matches_rule(rule: RateLimitRule, *, method: str, path: str) -> bool:
    ...
    if rule.methods and method.upper() not in rule.methods:
        return False
    if rule.routes:
        return any(path.startswith(prefix) for prefix in rule.routes)
    return True


def _parse_rules(payload: Iterable[Any]) -> Tuple[RateLimitRule, ...]:
    ...
    for idx, item in enumerate(payload):
        ...
        try:
            ...
            ValueError,
            KeyError,
        ) as exc:  # pragma: no cover - defensive against bad config
            LOGGER.warning("Failed to parse rate limit rule %s: %s", idx, exc)
    return tuple(sorted(rules, key=lambda rule: rule.priority))


def _merge_rules(
    base_rules: Sequence[RateLimitRule], extra_rules: Sequence[RateLimitRule]
) -> Tuple[RateLimitRule, ...]:
...
def load_rate_limiter_config(*, env: Mapping[str, str] | None = None) -> RateLimiterConfig:
    ...
    config.redis_prefix = (
        _get("RATE_LIMIT_REDIS_PREFIX", config.redis_prefix) or config.redis_prefix
    )
    ...
    config.forwarded_for_header = (
        _get("RATE_LIMIT_FORWARDED_FOR_HEADER", config.forwarded_for_header)
        or config.forwarded_for_header
    )
    config.identity_header = _get("RATE_LIMIT_IDENTITY_HEADER", config.identity_header)
    config.default_rule_name = (
        _get("RATE_LIMIT_DEFAULT_RULE_NAME", config.default_rule_name) or config.default_rule_name
    )
    config.default_limit = max(
        _as_int(env_map.get("RATE_LIMIT_DEFAULT_LIMIT"), config.default_limit), 1
    )
    config.default_window = max(
        _as_int(env_map.get("RATE_LIMIT_DEFAULT_WINDOW"), config.default_window), 1
    )
    config.default_scope = _coerce_scope(
        _get("RATE_LIMIT_DEFAULT_SCOPE", config.default_scope) or config.default_scope,
        config.default_scope,
    )
    config.default_priority = _as_int(
        env_map.get("RATE_LIMIT_DEFAULT_PRIORITY"), config.default_priority
    )
    ...
    extra_rules: Tuple[RateLimitRule, ...] = ()
    ...
    if rules_blob:
        try:
            ...
            if isinstance(parsed, list):
                extra_rules = _parse_rules(parsed)
...
class RateLimiter:
    ...
    async def consume(
        ...
        *,
        ...
        bucket = self._build_bucket(rule=rule, identity=identity, path=path)
        tokens = rule.cost if cost is None or cost == 0 else cost
...
_LIMITER_STATE: Dict[str, RateLimiter] = {}
...
def configure_rate_limiter(config: RateLimiterConfig | None = None) -> RateLimiter:
    ...
    instance = RateLimiter(resolved_config)
    _LIMITER_STATE["instance"] = instance
    return instance


def get_rate_limiter() -> RateLimiter:
    instance = _LIMITER_STATE.get("instance")
    if instance is not None:
        return instance
    instance = RateLimiter(load_rate_limiter_config())
    _LIMITER_STATE["instance"] = instance
    return instance
...
ApplyModuleOverrides = Any
apply_module_overrides: ApplyModuleOverrides | None
