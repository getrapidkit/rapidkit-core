...
def module_test_context_fixture(tmp_path: Path) -> dict[str, object]:
    """Provide an isolated workspace for Admin Console module tests."""

    working_dir = tmp_path / "admin_console"
...
@pytest.fixture()
def rendered_admin_console(tmp_path: Path) -> dict[str, object]:
    ...
    vendor_path = (
        ...
        / config["name"]
        / config["version"]
        / "src/modules/free/business/admin_console"
        / "admin_console.py"
    )
    return {
        "config": config,
        "root": tmp_path,
        "vendor": _load_module("generated_admin_console_vendor", vendor_path),
    }
