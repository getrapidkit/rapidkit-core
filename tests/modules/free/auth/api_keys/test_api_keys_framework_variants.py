"""Framework variant expectations for Api Keys."""
...
def test_fastapi_files_listed(module_config: dict[str, object]) -> None:
    ...
    assert any(
        entry.get("output") == "src/modules/free/auth/api_keys/api_keys.py" for entry in files
    )
...
def test_nestjs_files_listed(module_config: dict[str, object]) -> None:
    ...
    assert any(
        entry.get("output") == "src/modules/free/auth/api_keys/api-keys.service.ts"
        for entry in files
    )
