...
def test_generate_main_nestjs(generate_cli):
    ...
    middleware_dir = output_dir / "src" / "modules" / "free" / "essentials" / "middleware"
    expected = {
        middleware_dir / "service.ts",
        middleware_dir / "controller.ts",
        middleware_dir / "module.ts",
    }
    ...
    controller_src = (middleware_dir / "controller.ts").read_text()
    assert (
        "@Controller('middleware')" in controller_src
        or '@Controller("middleware")' in controller_src
    ), "Controller should expose middleware routes"
