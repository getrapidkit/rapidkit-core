...
def module_test_context_fixture(tmp_path: Path) -> dict[str, object]:
    """Provide an isolated workspace for Usage Billing module tests."""

    working_dir = tmp_path / "usage_billing"
...
@pytest.fixture()
def rendered_usage_billing(tmp_path: Path) -> dict[str, object]:
    ...
    return {"root": tmp_path, "vendor": _load_module("generated_usage_billing_vendor", vendor_path)}
