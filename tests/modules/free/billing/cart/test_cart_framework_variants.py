"""Framework variant expectations for Cart."""
...
def test_fastapi_files_listed(module_config: dict[str, object]) -> None:
    ...
    assert any(entry.get("output") == "src/modules/free/billing/cart/cart.py" for entry in files)
...
def test_nestjs_files_listed(module_config: dict[str, object]) -> None:
    ...
    assert any(
        entry.get("output") == "src/modules/free/billing/cart/cart.service.ts" for entry in files
    )
