...
@pytest.mark.asyncio
async def test_fastapi_routes_handle_crud(generated_modules) -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        response = await client.get("/api/cart/test")
        assert response.status_code == 200

        response = await client.post("/api/cart/test/discounts/WELCOME", json={"force": True})
        assert response.status_code == 200

        response = await client.delete("/api/cart/test/items/sku")
        assert response.status_code == 200
