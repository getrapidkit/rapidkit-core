...
@pytest.mark.asyncio
async def test_register_fastapi_applies_headers(fastapi_adapter) -> None:
...
@pytest.mark.asyncio
async def test_settings_override_csp(fastapi_adapter) -> None:
    runtime_module, _ = fastapi_adapter
    app = FastAPI()
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/security-headers/headers")
...
@pytest.mark.asyncio
async def test_routes_expose_health(fastapi_adapter) -> None:
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/security-headers/health")
