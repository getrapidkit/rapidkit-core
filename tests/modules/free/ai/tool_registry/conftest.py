...
def module_test_context_fixture(tmp_path: Path) -> dict[str, object]:
    """Provide an isolated workspace for Tool Registry module tests."""

    working_dir = tmp_path / "tool_registry"
...
@pytest.fixture()
def rendered_tool_registry(tmp_path: Path) -> dict[str, object]:
    ...
    return {
        "config": config,
        "root": tmp_path,
        "vendor": _load_module("generated_tool_registry_vendor", vendor_path),
    }
