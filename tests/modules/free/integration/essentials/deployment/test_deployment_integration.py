...
def test_deployment_scripts_structure():
    ...
    # Check for common deployment scripts (historic names) and new workflow helpers
    common_scripts = {
        "deploy.sh",
        "start.sh",
        "stop.sh",
        "health-check.sh",
        "deploy-workflows.sh",
        "deploy-workflows-auto.sh",
        "setup-dev.sh",
        "setup-dev-environment.sh",
    }

    existing_scripts = [
        script_name for script_name in common_scripts if (scripts_dir / script_name).exists()
    ]
    ...
    if not existing_scripts:
        automation_scripts = [
            ...
            if any(
                keyword in path.stem for keyword in ("deploy", "start", "stop", "health", "setup")
            )
        ]
        automation_scripts.extend(
            path
            for path in scripts_dir.glob("*.py")
            if any(keyword in path.stem for keyword in ("deploy", "start", "stop", "health"))
        )
        assert automation_scripts, "No deployment automation scripts found in scripts/"
...
def test_deployment_config_files():
    ...
    assert len(existing_files) > 0, (
        f"No deployment configuration files found. " f"Expected at least one of: {config_files}"
    )
