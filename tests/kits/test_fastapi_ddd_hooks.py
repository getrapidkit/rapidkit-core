...
from kits.fastapi.ddd import hooks
...
def test_pre_generate_injects_defaults(
    ...
    assert fresh_variables["description"].startswith("Domain-driven FastAPI service")
...
def test_post_generate_prints_summary(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "FastAPI DDD project scaffolded" in captured
...
def test_post_generate_falls_back_to_default_name(
    ...
    captured = capsys.readouterr().out
    expected = (variables or {}).get("project_name", "fastapi-ddd-service")
